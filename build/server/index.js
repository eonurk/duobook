...
try {
    ...
    console.error("Server startup failed. Ensure serviceAccountKey.json is configured correctly.");
...
app.post("/api/generate-story", asyncHandler(async (req, res, next) => {
    ...
    if (!description || !source || !target || !difficulty || !length) {
        const err = new Error("Missing required parameters for story generation.");
        err.status = 400;
        return next(err);
    }
    ...
    try {
        ...
        err.status = 502; // Bad Gateway or appropriate error
        next(err);
    }
}));
// === STORY ROUTES ===
// GET all stories for the logged-in user - Wrapped with asyncHandler
app.get("/api/stories", asyncHandler(async (req, res, next) => {
    // Added next param
...
app.post("/api/stories", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401; // Add status for error handler
        return next(err);
    }
    // Destructure all expected fields from the body
    const { story, description, sourceLanguage, targetLanguage, difficulty, length, } = req.body;
...
app.get("/api/stories/:id", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401;
...
app.delete("/api/stories/:id", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401;
    ...
    try {
        ...
        // Verify ownership
        if (story.userId !== userId) {
            const err = new Error("You do not have permission to delete this story.");
            err.status = 403;
            return next(err);
        }
        // Delete the story
...
function isYesterday(targetDate, today) {
    if (!targetDate || !today)
        return false;
...
app.get("/api/progress", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401;
    ...
    try {
        ...
        if (!progress.lastLogin || !isSameDay(progress.lastLogin, now)) {
            ...
            if (progress.lastLogin && isYesterday(progress.lastLogin, now)) {
                ...
                console.log(`User ${userId} continued streak. New streak: ${newStreak}`);
            }
            else {
...
app.put("/api/progress", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401;
    ...
    try {
        ...
        if (Object.keys(updateData).length === 0) {
            const err = new Error("No update data provided (pointsToAdd or level).");
            err.status = 400;
            return next(err);
        }
        // Use updateData here
...
app.get("/api/achievements", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401;
...
app.get("/api/user/achievements", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required (userId missing after auth).");
        err.status = 401;
...
function isSameDay(date1, date2) {
    if (!date1 || !date2)
        return false; // Handle null/undefined dates
...
app.get("/api/challenges", asyncHandler(async (req, res, next) => {
    try {
...
app.get("/api/user/daily-challenges", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required.");
        err.status = 401;
    ...
    try {
        ...
        if (userProgress.dailyChallengesLastGenerated &&
            isSameDay(userProgress.dailyChallengesLastGenerated, today)) {
            needsNewChallenges = false;
        }
        if (needsNewChallenges) {
            console.log(`Generating new daily challenges for user ${userId} for day ${todayStart.toISOString()}`);
            ...
            const coreChallenges = allChallenges.filter((c) => c.isCore);
            const randomChallenges = allChallenges.filter((c) => !c.isCore);
            // Determine how many random challenges to pick (aim for 3 total)
            const totalDesired = 3;
            const numRandomNeeded = Math.max(0, totalDesired - coreChallenges.length);
            // Shuffle and select random challenges
            const shuffledRandom = randomChallenges.sort(() => 0.5 - Math.random());
            const selectedRandom = shuffledRandom.slice(0, Math.min(numRandomNeeded, randomChallenges.length));
            ...
            const transactionResult = await prisma.$transaction(async (tx) => {
                ...
                const createPromises = selectedChallenges.map((challenge) => tx.userDailyChallenge
                    .create({
                    data: {
                        userId: userId,
                    ...
                    if (e.code === "P2002") {
                        ...
                        console.warn(`Challenge ${challenge.id} already exists for user ${userId} on ${todayStart.toISOString()}. Skipping.`);
            ...
            console.log(`Generated/retrieved ${transactionResult.length} challenges.`);
            res.status(200).json(transactionResult);
        }
        else {
            console.log(`Returning existing daily challenges for user ${userId} for day ${todayStart.toISOString()}`);
            res.status(200).json(userProgress.userDailyChallenges);
        }
    }
    catch (error) {
        console.error(`Error fetching/generating daily challenges for user ${userId}:`, error);
        next(error);
    }
}));
// POST progress towards a specific challenge TYPE
app.post("/api/user/challenges/progress", asyncHandler(async (req, res, next) => {
    const userId = req.userId;
    if (!userId) {
        const err = new Error("Authentication required.");
        err.status = 401;
    ...
    try {
        ...
        console.error(`Error updating challenge progress for type ${challengeType} for user ${userId}:`, error);
        next(error);
    }
}));
// --- ERROR HANDLING ---
