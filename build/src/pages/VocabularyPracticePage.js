...
function VocabularyPracticePage() {
    ...
    // -------------------------------
    if (isLoading) {
        return (_jsxs("div", { className: "container mx-auto py-10 px-4 text-center", children: [_jsx(Loader2, { className: "h-8 w-8 animate-spin mx-auto mb-2" }), "Loading vocabulary..."] }));
    }
    if (error) {
        return _jsxs("div", { className: "container mx-auto py-10 px-4 text-center text-red-600", children: ["Error: ", error] });
    }
    if (!currentUser) {
        return _jsx("div", { className: "container mx-auto py-10 px-4 text-center text-muted-foreground", children: "Please log in to practice vocabulary." });
    }
    if (shuffledList.length === 0 && !isLoading) {
        return _jsx("div", { className: "container mx-auto py-10 px-4 text-center text-muted-foreground", children: "No vocabulary found or not enough words to start a quiz (minimum 4 needed). Generate some stories first!" });
    }
    // --- Render Quiz UI ---
    const currentWordItem = shuffledList[currentQuestionIndex];
    return (_jsxs("div", { className: "container mx-auto py-10 px-4 max-w-2xl", children: [_jsx("h1", { className: "text-3xl font-bold mb-6 text-center", children: "Vocabulary Practice" }), !quizCompleted && shuffledList.length > 0 && (_jsxs("div", { className: "w-full bg-muted rounded-full h-2.5 mb-4 overflow-hidden", children: [" ", _jsx("div", { className: "bg-primary h-2.5 rounded-full transition-all duration-500 ease-out", style: { width: `${progressPercent}%` } }), " "] })
            ...
            */
            ), quizCompleted ? (_jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { className: "text-center", children: "Quiz Complete!" }) }), _jsxs(CardContent, { className: "text-center", children: [_jsxs("p", { className: "text-xl mb-2", children: ["Your final score: ", score, " / ", shuffledList.length] }), score >= 8 ? (_jsxs("p", { className: "text-lg mb-4 text-primary", children: ["+$", COMPLETION_BONUS_XP, " XP High Score Bonus!"] })) : (_jsx("p", { className: "text-sm mb-4 text-muted-foreground", children: "(Score 8+ required for bonus XP)" })), _jsxs("div", { className: "flex justify-center gap-4", children: [_jsxs(Button, { onClick: handleRestartQuiz, variant: "secondary", children: [_jsx(RotateCcw, { className: "mr-2 h-4 w-4" }), " Restart Quiz"] }), incorrectAnswers.length > 0 && (_jsxs(Button, { onClick: () => setShowReview(prev => !prev), variant: "outline", children: [showReview ? 'Hide' : 'Review Mistakes', " (", incorrectAnswers.length, ")"] }))] }), showReview && incorrectAnswers.length > 0 && (_jsxs("div", { className: "mt-6 pt-4 border-t text-left", children: [_jsx("h3", { className: "text-lg font-semibold mb-3 text-center", children: "Words to Review:" }), _jsx("ul", { className: "space-y-2 list-disc list-inside", children: incorrectAnswers.map((item, idx) => (_jsxs("li", { className: "text-sm", children: [_jsxs("span", { className: "font-medium", children: [item.word, ":"] }), " ", item.translation] }, idx))) })] }))] })] })) : currentWordItem ? (_jsxs(Card, { children: [_jsxs(CardHeader, { children: [_jsx(CardTitle, { children: "Translate the word:" }), _jsxs("p", { className: "text-sm text-muted-foreground text-right", children: ["Word ", currentQuestionIndex + 1, " of ", shuffledList.length, " | Score: ", score] })] }), _jsxs(CardContent, { children: [_jsx("p", { className: "text-2xl font-semibold mb-6 text-center p-4 bg-primary/10 text-primary rounded", children: currentWordItem.word }), _jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4", children: currentOptions.map((option, idx) => (_jsx(Button, { variant: "outline", className: cn("w-full justify-center h-auto py-3 px-4 text-base whitespace-normal text-left transition-all duration-300 ease-in-out transform", 
                                    ...
                                    isAnswered && option !== currentWordItem?.translation && option !== selectedAnswer &&
                                        "opacity-60 border-muted", !isAnswered && "hover:bg-accent hover:scale-105" // Hover effect before answer
                                    ), onClick: () => handleOptionClick(option), disabled: isAnswered, children: option }, idx))) }), _jsxs("div", { className: "mt-6 text-center", children: [_jsx("p", { className: cn("h-auto min-h-[1.5rem] mb-2 font-semibold animate-fade-in", // Allow wrapping, ensure min height
                                        isAnswered && feedback.startsWith("Correct") && "text-green-600", isAnswered && feedback.startsWith("Incorrect") && "text-destructive"), "aria-live": "polite" // Accessibility: Announce feedback changes
                                        , children: feedback }, animateTrigger), isAnswered && (_jsx(Button, { onClick: handleNextQuestion, variant: "secondary", className: "mt-2 w-full flex items-center justify-center gap-2 bg-slate-200 text-primary-foreground hover:bg-primary/90 font-medium text-sm py-6", children: currentQuestionIndex === shuffledList.length - 1
                                            ? _jsxs(_Fragment, { children: [_jsx(CheckCircle, { className: "mr-2 h-4 w-4" }), " Finish Quiz"] })
                                            : _jsxs(_Fragment, { children: ["Next Word", _jsx(ArrowRight, { className: "h-4 w-4" })] }) }))] })] })] })) : (_jsx("p", { className: "text-center text-muted-foreground", children: "Preparing quiz..." }) // Should be brief
            )] }));
}
