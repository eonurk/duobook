...
function VocabularyPracticePage() {
			...
			getStories()
				.then((storiesFromApi) => {
					const allVocab = storiesFromApi.reduce((acc, story) => {
						try {
							...
							if (storyData?.vocabulary) {
								...
								const validVocab = storyData.vocabulary.filter(
									(v) => v && v.word && v.translation
								);
								acc.push(...validVocab);
							}
						} catch (e) {
					...
					if (uniqueVocab.length >= 4) {
						...
						setError(
							"Not enough unique vocabulary words found across your stories to start a quiz (minimum 4 needed)."
						);
						setShuffledList([]);
					}
				})
	...
	const setupQuestion = (index, list) => {
		if (!list || index >= list.length) {
			...
			// Track completion of quiz
			const targetLanguage =
				shuffledList.length > 0 && shuffledList[0].targetLanguage
					? shuffledList[0].targetLanguage
					: "Unknown";
			trackPractice(targetLanguage, shuffledList.length);
			return;
		}
		...
		if (awardedPoints > 0 && userProgress && updateProgressData) {
			try {
				await updateProgressData({
					points: userProgress.points + awardedPoints,
				});
	...
	const handleNextQuestion = async () => {
		// Make async for completion bonus
		...
		if (nextIndex < shuffledList.length) {
			...
			if (score >= 8 && userProgress && updateProgressData) {
				try {
					await updateProgressData({
						points: userProgress.points + COMPLETION_BONUS_XP,
					});
					console.log(
						`Awarded ${COMPLETION_BONUS_XP} completion bonus points for high score!`
					);
				} catch (error) {
					console.error(
						"Failed to save completion bonus points update:",
						error
					);
				}
			}
	...
	if (shuffledList.length === 0 && !isLoading) {
		return (
			<div className="container mx-auto py-10 px-4 text-center text-muted-foreground">
				No vocabulary found or not enough words to start a quiz (minimum 4
				needed). Generate some stories first!
			</div>
		);
	}
				...
				<Card>
					...
					<CardContent className="text-center">
						<p className="text-xl mb-2">
							Your final score: {score} / {shuffledList.length}
						</p>
				...
				<Card>
					<CardHeader>
						...
						<p className="text-sm text-muted-foreground text-right">
							Word {currentQuestionIndex + 1} of {shuffledList.length} | Score:{" "}
							{score}
						</p>
					</CardHeader>
					<CardContent>
							...
							{currentOptions.map((option, idx) => (
								<Button
									...
									className={cn(
										...
										isAnswered &&
											option === currentWordItem?.translation &&
						...
						<div className="mt-6 text-center">
							...
							{isAnswered && (
								<Button
									...
									variant="secondary"
									className="mt-2 w-full flex items-center justify-center gap-2 bg-slate-200 text-primary-foreground hover:bg-primary/90 font-medium text-sm py-6"
								>
