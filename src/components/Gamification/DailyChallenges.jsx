...
function DailyChallenges() {
  ...
  if (error) {
      return <div className="mt-8 text-center text-red-500">Error loading challenges: {error}</div>;
  }
  ...
  return (
    <div className="mt-8">
      ...
      {challenges.length > 0 ? (
          ...
          {challenges.map((userChallenge) => {
            ...
            return (
              <div
                ...
                className={`bg-card text-card-foreground rounded-lg border shadow-sm p-4 border-l-4 ${completed ? 'border-green-500 opacity-70' : 'border-primary'}`}
              >
                <div className="flex items-start">
                  <div className="text-2xl mr-3">{getChallengeIcon(challenge.type)}</div>
                  <div className="flex-1">
                    <h3 className="font-semibold">{challenge.title}</h3>
                    <p className="text-sm text-muted-foreground mb-2">{challenge.description}</p>

                    {requiredCount != null && (
                      <div className="w-full bg-muted rounded-full h-1.5 mb-2 dark:bg-gray-700">
                        <div
                          className={`h-1.5 rounded-full ${completed ? 'bg-green-500' : 'bg-primary'}`}
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-primary">+{challenge.xpReward} points</span>
                      {completed ? (
                        <span className="text-xs text-green-500 flex items-center">
                         ...
                         <span className="text-xs text-muted-foreground">{progress}/{requiredCount}</span>
                      ) : (
                         <span className="text-xs text-muted-foreground">Complete the action</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <p className="text-muted-foreground text-center py-4">No daily challenges assigned for today. Check back tomorrow!</p>
      )}
